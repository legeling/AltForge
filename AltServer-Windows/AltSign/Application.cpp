...
std::map<std::string, plist_t> Application::entitlements()
{
	if (_entitlements.size() == 0)
	{
		...
		{
			odslog("Error parsing application entitlements.");
		}
	}
...
{
	auto isAltStoreApp = this->bundleIdentifier().find("com.legeling.AltForge") != std::string::npos ||
		this->bundleIdentifier().find("com.rileytestut.AltStore") != std::string::npos;
