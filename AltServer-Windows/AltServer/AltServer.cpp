...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case (WM_MENUSELECT):
	{
		...
		if (alreadyFetched)
		{
			...
			odslog("Installed apps already fetched; ignoring duplicate request.");
			return 0;
		}

		HMENU appsMenu = GetSubMenu(_enableJITMenu, index);
		HMENU cachedEnableJITMenu = _enableJITMenu;
		odslog("Fetching installed apps...");
		...
		.then([appsMenu, cachedEnableJITMenu, index, device](std::vector<InstalledApp> installedApps) {
			...
			odslog("Fetched " << installedApps.size() << " installed apps.");
	...
	case (WM_USER + 1):
	{
		...
		case WM_LBUTTONUP:
		{
			...
			const wchar_t* installTitle = isSideloadingIPA ? L"Sideload .ipa" : L"Install AltForge";
