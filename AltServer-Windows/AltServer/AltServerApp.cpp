...
const char* REGISTRY_ROOT_KEY = "SOFTWARE\\AltForge\\AltServer";
...
std::wstring altstoreSourceURL = L"https://github.com/legeling/AltForge/releases/latest/download/apps.json";
std::wstring altstoreBundleID = L"com.legeling.AltForge";
...
static int CALLBACK BrowseFolderCallback(HWND hwnd, UINT uMsg, LPARAM lParam, LPARAM lpData)
{
	...
	{
		odslog("Initialized folder browser.");
...
void AltServerApp::Start(HWND windowHandle, HINSTANCE instanceHandle)
{
	...
	try
	{
		...
		{
			this->ShowAlert("iCloud Not Installed", "iCloud must be installed from Apple's website (not the Microsoft Store) in order to use AltForge.");
		}
#endif
	}
...
{
	ShellExecuteW(NULL, L"open", L"https://github.com/legeling/AltForge/releases/latest", NULL, NULL, SW_SHOWNORMAL);
}
...
{
    auto appName = filepath.has_value() ? fs::path(*filepath).filename().string() : "AltForge";
    ...
    .then([=](std::shared_ptr<Certificate> tempCertificate)
          {
				*certificate = *tempCertificate;
                ...
                return this->PrepareDevice(device).then([=](pplx::task<void> task) {
                    ...
                    {
                        odslog("Failed to install DeveloperDiskImage.dmg. " << error.localizedDescription());
                    }
                    catch (std::exception& exception)
                    {
                        odslog("Failed to install DeveloperDiskImage.dmg. " << exception.what());
                    }
                    ...
                    else
                    {
						...
						// Show alert before downloading AltForge.
						this->ShowInstallationNotification("AltForge", device->name());
...
pplx::task<std::string> AltServerApp::FetchAltStoreDownloadURL(std::shared_ptr<Device> device)
{
	...
	{
		odslog("Received AltForge source response status code: " << response.status_code());
	...
	.then([=](json::value json) {
		try
		{
			...
			{
				throw ServerError(ServerErrorCode::UnsupportediOSVersion, { {AppNameErrorKey, "AltForge"}, {OperatingSystemNameErrorKey, osName}, {OperatingSystemVersionErrorKey, minOSVersionString} });
			}
			...
			else
			{
				...
				oss << device->name() << " is running " << osName << " " << device->osVersion().stringValue() << ", but AltForge requires " << osName << " " << minOSVersion << " or later.";
				oss << "\n\n";
				oss << "Would you like to download the last version compatible with this device instead (AltForge " << StringFromWideString(latestSupportedVersionNumber) << ")?";
...
pplx::task<std::shared_ptr<Team>> AltServerApp::FetchTeam(std::shared_ptr<Account> account, std::shared_ptr<AppleAPISession> session)
{
    ...
    .then([](std::vector<std::shared_ptr<Team>> teams) {
		...
		for (auto& team : teams)
		{
			if (team->type() == Team::Type::Organization)
			{
				return team;
			}
		}
			...
			for (auto& certificate : certificates)
			{
				...
				if (fs::exists(cachedCertificatePath) && certificate->machineIdentifier().has_value())
				{
					...
					catch(std::exception &e)
					{
						...
						odslog("Failed to load cached certificate. " << e.what())
					}
				}
                            ...
                            .then([privateKey, addedCertificate, cachedCertificatePath](std::vector<std::shared_ptr<Certificate>> certificates)
                                {
									...
									catch (std::exception& e)
									{
										...
										odslog("Failed to cache certificate. " << e.what())
									}
	...
	return pplx::create_task([=]() -> pplx::task<std::shared_ptr<AppID>> {
		...
		.then([=](std::vector<std::shared_ptr<AppGroup>> fetchedGroups) {
			...
			for (auto groupIdentifier : applicationGroups)
			{
				...
				{
					std::string name = std::regex_replace("AltForge " + groupIdentifier, std::regex("\\."), " ");
...
If you already have iCloud installed, please locate the "Apple" folder that was installed with iCloud. This can normally be found at:
		...
		else if (result == ID_FOLDER)
		{
			...
			odslog("Selected Apple dependency folder.");
...
fs::path AltServerApp::appDataDirectoryPath() const
{
	...
	altserverDirectoryPath.append("AltForge");
