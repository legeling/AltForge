...
static void mkdir_p(const std::string &path) {
    ...
    auto bslash(path.rfind('\\', path.size() - 1));
    if (bslash != std::string::npos && (slash == std::string::npos || bslash > slash))
        slash = bslash;
    if (slash == std::string::npos)
        return;
...
#ifdef __WIN32__
    _syscall(remove(path.c_str()), ENOENT);
#endif
...
DiskFolder::DiskFolder(const std::string &path) :
    path_(path)
{
    _assert_(path_.size() == 0 || path_[path_.size() - 1] == '\\' || path_[path_.size() - 1] == '/', "missing path separator on %s", path_.c_str());
}
...
// AltServer: custom helper to locate executable path from app bundle
...
// AltServer: extract entitlements XML from a signed binary or app bundle
