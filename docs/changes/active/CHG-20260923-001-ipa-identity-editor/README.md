...
- The v2.5.0 tag CI compiled the iOS target and passed the other three new editor tests, but `testIPAIdentityEditorRewritesMainExtensionsAndLocalizedName` failed twice without an assertion in the console log. The Apple job failed and no Draft or public Release was created. The identical fixture passed a macOS harness using the actual editor and AltSign framework; that does not establish the iOS result. A manually dispatched branch preflight now reports the xcresult failure summary before another release tag is attempted.
