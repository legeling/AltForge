...
- [x] GitHub Windows runner 完成 Release build 和 ZIP 打包。
...
- `actionlint 1.7.7` 已检查初始 CI/Release workflows；五项 Release 产物 metadata/checksum fixture 通过。七次 hosted Windows run 依次暴露 runner vcpkg 漂移、`cpprestsdk` 被 deindex、Apple mDNSResponder Windows stub 未定义 `LOG_ERR`、完整 solution compiler/include compatibility、两处依赖 Windows `min` macro 的调用、zlib 1.3.2 的 Windows library/runtime 重命名，以及自定义 install root 下缺少 cpprestsdk 显式链接。第七次 `windows-2025` job 已以固定依赖和最小 compiler workarounds 完成 Win32 Release build 及 ZIP 打包。
