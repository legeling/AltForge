...
2026-09-23 follow-up: A local candidate in CHG-20260923-002 moves installation status into the scrolling My Apps list and asks the updated macOS/Windows Server for an authoritative installed-bundle-ID match when foreground UTI registration remains inconclusive. It preserves the receipt on absent/old Server or negative lookup; only a positive device answer can restore a record. The cross-platform build and locked-device refresh smoke are not yet accepted. A running installation cannot be guaranteed to continue indefinitely after iOS suspends the app.
