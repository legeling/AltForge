...
2026-09-23 follow-up: CHG-20260923-002 moves installation status into the scrolling My Apps list and asks the updated macOS/Windows Server for an authoritative installed-bundle-ID match when foreground UTI registration remains inconclusive. It preserves the receipt on absent/old Server or negative lookup; only a positive device answer can restore a record. Manual CI preflight 35828141331 passed iOS XCTest, Apple build/package checks and Windows build; locked-device install and refresh smoke remain open. A running installation cannot be guaranteed to continue indefinitely after iOS suspends the app.
