...
| `T-040` 修复 Apple 认证响应格式失败 | User-confirmed macOS login / 2.4.5 release preparation | `FR-041`; `DES-027`; `TEST-040` | CHG-20260905-002 移植上游 #47/#49 的 AuthKit 身份、独立会话与有限重试；本机 17 项 XCTest、Apple 构建及测试包签名检查通过。用户确认 macOS 登录成功并进入设备准备阶段，授权发布 2.4.5；完整安装与跨平台账号覆盖继续由 ISSUE-20260904-001 跟踪 |
