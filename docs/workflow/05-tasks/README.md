...
| `T-040` 修复 Apple 认证响应格式失败 | Follow-up local gates passed / release pending | `FR-041`; `DES-027`; `TEST-040` | `v2.4.3 (21)` 已使用现代身份且能把 Cocoa 3840 收口为 3020，但用户仍在约 2.5 秒内登录失败；上游同时捕获到 `apptokens` 返回 HTML 503。follow-up 已统一全部认证 plist parser，保留脱敏 operation/status/MIME，并以 429/5xx/HTML 定向提示覆盖；repository contract、3 项 XCTest、iOS Simulator 与 Universal macOS Debug build 通过，真实登录仍待新版本复测 |
