...
| `T-006` 维护错误序列化回归 | Done | `FR-011`; `DES-008`; `TEST-010` | 现有 AltTests 覆盖本地/远端 error bridge |
| `T-007` 为 release metadata 添加 dry-run contract test | Pending | `FR-015`, `FR-016`; `DES-010`; `TEST-013` | size/hash/schema/缺失参数均有自动化断言 |
...
| `T-010` 移植低风险上游维护修复 | In progress | `FR-008`, `FR-011`, `FR-017`; `DES-003`, `DES-008`; `TEST-016`, `TEST-017` | 组织团队 fallback、到期天数下限、错误详情选择完成构建验证 |
