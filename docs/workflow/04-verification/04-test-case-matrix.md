...
|---|---|---:|---|---|---|
| `TEST-048` | iOS/macOS local IPA install and icon crop | P1 | Fixture/XCTest/Contract/Build/Device | visible desktop menu, direct/edit on both clients, macOS non-square crop/zoom, iOS photo crop, 1024/120/180/152 icon outputs and iPhone/iPad metadata, IDs, source unchanged, cleanup and real-device refresh | Local fixture/build/typecheck/contracts, hosted preflight 35985608497 and v2.6.0 tag CI 35987685997 pass; downloaded checksums pass; device pending |
...
| `TEST-043` | iOS installed-app tracking recovery | P0 | Unit/Contract/Device | receipt before send、Core Data immediate save、positive identity only、delayed registration、lifecycle retry bounds、live-producer deferral、synchronized progress reads、idempotency、save failure rollback、existing version preservation、safe cache retention、explicit removal | Local tests/build passed; device matrix pending |
| `TEST-044` | iOS installation progress and themes | P1 | Unit/Layout/Device | safe-area panel、large text/phone/landscape、terminal result、idle leases、cancel/timeout、theme switches、semantic colors | Local unit/layout passed; real lock/timeout and whole-app visual matrix pending |
