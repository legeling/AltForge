...
| Apple authentication response compatibility | `FR-041` | `DES-027` | `TEST-040` | Published `v2.4.3 (21)` confirms the modern identity and base 3020 mapping but still fails the user's live login. Follow-up single parser path, safe operation/status/MIME metadata, targeted 429/5xx/HTML presentation, repository contract, 3 XCTest cases, iOS Simulator and Universal macOS Debug builds pass; release and real-account verification remain pending | `T-040` |
