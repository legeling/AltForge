...
|---|---|---|---|---|---|
| iOS/macOS IPA install options and icon crop | `FR-049` | `DES-034` | `TEST-048` | Native macOS fixture/build, iOS SDK typecheck and static/release contracts pass; CI iOS XCTest and device install/icon/refresh pending. | `T-047` |
...
| Apple authentication response compatibility | `FR-041` | `DES-027` | `TEST-040` | CHG-20260905-002 published in v2.4.5 (23): AuthKit identity, isolated sessions, bounded retries, structured error priority and privacy. All 17 local/hosted XCTest, three-platform Release builds, downloaded checksums, package identities/versions/signature and public latest readback passed. User confirmed macOS login and device preparation. Full installation, other platform logins and the 2FA matrix remain unverified. | `T-040` |
...
| macOS direct update download | `FR-043` | `DES-029` | `TEST-042` | Source/static contract, local/hosted Apple and Windows builds, full live DMG probes, published `v2.4.3 (21)` artifact checksums/binary/localization inspection and latest endpoint readback pass; update-available progress/cancel/collision/failure/automatic DMG-open UI still needs a local older-version build | `T-042` |
