...
iOS 安装崩溃、恢复日志或认证页面变化执行 `TEST-035`：先检查脱敏系统 crash report 的异常类型和首个业务栈帧，再运行 repository contract、`Scripts/test_ldid_architecture_compatibility.sh`、Watch 目录移除/签名 detail 清理/畸形 plist XCTest、catalog JSON 解析、Settings/Authentication storyboard/XIB XML 解析和 iOS Simulator build。ldid 回归必须对 Xcode watchOS SDK 生成的最小 `arm64_32` Mach-O 完成签名，并证明未知 CPU type、nil entitlement 和非法路径返回可捕获错误而不是 `SIGSEGV`；synthetic app 必须证明签名前只从副本删除顶层 `Watch/`，重复删除幂等。静态检查不得残留固定白色设置文字、运行时递归改色、在 `viewIsAppearing` reload 后对动态 no-updates index path 调用 `reconfigureItems`，或在 flow-layout 尺寸计算中 dequeue supplementary view。在同一 Simulator 的深浅色模式至少执行 6 轮“设置 -> 我的 App -> 设置”切换，并在安装失败后额外执行 10 轮“我的 App”往返，覆盖 no-updates section 为 0/1 item 和有/无 App IDs footer；确认正文可读、进程不变且没有新 crash report。验证工作原理的 USB/Wi-Fi、团队选择、签名和七天刷新说明。另覆盖 operation 无 success/error 和 prepared app 缺失路径，必须返回普通错误而不是 precondition 或模糊的 invalid parameters。遗留 operation record 最多保留 20 条，每条最多 16 个阶段、单条 detail 最多 120 字符并通过原子 journal 落盘；连续签名事件必须只替换最后一个 checkpoint，失败日志显示诊断编号、最后非终态阶段、最新的最多四段 bundle 相对路径/架构和相对耗时轨迹，并能复制有界报告。前台异常退出只保留 current/interrupted 两条 checkpoint，且存在 operation 日志时不得重复生成 runtime 日志。fixture 必须证明不记录凭据、UDID、团队或 Server ID、证书/profile 和绝对路径，记录只在日志成功保存后消费。真实 IPA 的签名、发送、安装、失败日志和清理由 Suite E 完成，Simulator 结果不得替代设备 E2E。
