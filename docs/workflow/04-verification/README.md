...
| `TEST-040` Apple 认证响应兼容 | `FR-041` | Static/Unit/Build/Manual E2E | CHG-20260905-002 的 AuthKit 身份及有限独立会话重试已通过本机 17 项 XCTest（含 4 项生产传输回归）、iOS 测试宿主与 macOS Universal 构建、测试 App 签名检查。用户确认 macOS 登录恢复并进入设备准备阶段，已授权发布 2.4.5；完整安装、其他平台登录及 2FA 覆盖仍待补齐。 |
