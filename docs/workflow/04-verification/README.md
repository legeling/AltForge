...
| Test | 覆盖 | 层级 | 当前状态 |
|---|---|---|---|
| `TEST-048` 跨端 IPA 安装与图标裁切 | `FR-049` | Fixture/XCTest/Contract/Build/Device | 非正方形裁切/缩放 fixture、macOS Debug build、iOS SDK 类型检查、静态与发布契约通过；iOS XCTest 与三平台发布 CI 待跑，真机双路径、图标呈现及刷新待验收 |
...
| `TEST-040` Apple 认证响应兼容 | `FR-041` | Static/Unit/Build/Manual E2E | v2.4.5 (23) 已正式发布；本地及 hosted 17 项 XCTest、三平台 Release 构建、下载校验、IPA/DMG/Windows 版本、macOS 签名及 public latest 回读通过。用户确认 macOS 登录恢复并进入设备准备阶段；完整安装、其他平台登录及 2FA 覆盖仍待补齐。 |
