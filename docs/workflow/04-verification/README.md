...
| `TEST-040` Apple 认证响应兼容 | `FR-041` | Static/Build/Manual E2E | `v2.4.3 (21)` 实机反馈证明现代身份和基础 3020 映射尚未恢复登录；上游同期报告 `init`/`complete` 成功后 `apptokens` 返回 HTML 503。follow-up 的统一认证 parser、operation/status/MIME allowlist、repository contract、3 项定向 XCTest、iOS Simulator Debug 与 Universal macOS Debug build 通过；新构建复测前不能断言用户本次响应就是 503。真实账号登录、2FA、团队/证书查询与安装仍待执行。 |
