...
| `TEST-040` Apple 认证响应兼容 | `FR-041` | Static/Build/Manual E2E | 2026-09-04 在 macOS 26.5.2 / Xcode 26.6 定位：运行中的 AltForge Server 2.4.0 仍发送 Xcode 11 client identity；上游 #1747 的同错误响应以 `<html>` 开头，上游 #1772 确认该旧身份从九月初被 Apple 集中拒绝。静态 contract 覆盖共享的 Xcode 27 beta 6 产品/bundle identity、动态 CFNetwork/Darwin User-Agent、旧版本拒绝、畸形 plist 映射为 3020 及响应正文不记录；macOS AltServer 与 iOS Simulator build 通过，产物包含 `27.0 (27A5252f)` / `25183.54.10` 且不含 `3594.4.19`。无凭据探测不作为认证通过证据；真实账号登录、2FA、团队/证书查询与安装仍待执行。 |
| `TEST-041` 错误码与用户提示 | `FR-042` | XCTest/Static/Build/Localization | repository contract、iOS 288/macOS 198 条错误文案 zh-Hans 扫描、AltStore build-for-testing、两项定向 XCTest 与 AltServer Debug build 通过；真实简体中文界面与 Windows MSBuild/设备验证待执行。 |
