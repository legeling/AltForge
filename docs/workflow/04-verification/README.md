...
| `TEST-040` Apple 认证响应兼容 | `FR-041` | Static/Build/Manual E2E | 2026-09-04 在 macOS 26.5.2 / Xcode 26.6 定位：运行中的 AltForge Server 2.4.0 仍发送 Xcode 11 client identity；上游 #1747 的同错误响应以 `<html>` 开头，上游 #1772 确认该旧身份从九月初被 Apple 集中拒绝。静态 contract 覆盖现代客户端版本、旧版本拒绝、畸形 plist 映射为 3020 及响应正文不记录；macOS AltServer 与 iOS Simulator build 通过，Universal Server 二进制只含现代版本，不含凭据的 GSA probe 返回 200/plist。真实账号登录、2FA、团队/证书查询与安装仍待执行。 |
