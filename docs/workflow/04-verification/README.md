...
| `TEST-040` Apple 认证响应兼容 | `FR-041` | Static/Build/Manual E2E | 2026-09-04 在 macOS 26.5.2 / Xcode 26.6 定位：运行中的 AltForge Server 2.4.0 仍发送 Xcode 11 client identity；上游 #1747 的同错误响应以 `<html>` 开头，上游 #1772 确认该旧身份从九月初被 Apple 集中拒绝。静态 contract 覆盖共享的 Xcode 27 beta 6 产品/bundle identity、动态 CFNetwork/Darwin User-Agent、旧版本拒绝、畸形 plist 映射为 3020 及响应正文不记录；本地和 hosted Apple build、9 项 XCTest、Windows build、打包/checksum/latest 回读通过，公开 `v2.4.2 (20)` 产物包含新身份且不含 `3594.4.19`。无凭据探测不作为认证通过证据；真实账号登录、2FA、团队/证书查询与安装仍待执行。 |
...
| `TEST-042` macOS 直接下载更新 | `FR-043` | Static/Build/Network/Manual UI | repository contract、Swift/PBX/catalog parse、AltServer Release build 与公开 `v2.4.2` metadata/完整 DMG 下载 probe 通过，实测 size/SHA-256 匹配；低版本构建的下载进度、取消、重名复用、失败重试和自动挂载 GUI 矩阵待执行。 |
