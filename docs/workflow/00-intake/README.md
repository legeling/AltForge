...
1. 用户在 macOS 上运行 AltServer，把 AltForge 安装到已连接设备。
...
5. 维护者通过 CI 构建未签名 IPA、macOS AltServer、source JSON 和校验和，并用版本标签发布。
...
- 编译需要 Xcode、CocoaPods、Swift Package 依赖和递归 submodule。
...
- CI 能在受控超时内构建 iOS Simulator 和 macOS 目标，并运行关键自动化测试。
