...
| `FR-046` | `DES-031` | `TEST-045` | `T-044` |
| `FR-047` | `DES-032` | `TEST-046` | `T-045` |
| `FR-048` | `DES-033` | `TEST-047` | `T-046` |
| `FR-049` | `DES-034` | `TEST-048` | `T-047` |
...
- `FR-046`：官方 source 必须声明已审核的实际隐私权限，发布前核对真实 IPA 主 App/扩展与生成 source，漏声明即失败，不得自动放宽客户端检查。201 提示须区分源声明缺失与文件损坏，保留权限详情并指导刷新源或联系维护者。
- `FR-047`：选择本地 IPA 或导入链接后，安装前显示应用名称、包标识符、版本和扩展数量，提供“直接安装”与“按修改安装”两条路径。修改路径允许独立编辑显示名称与包标识符，并在签名及注册 App ID 前同步主 App、保留的扩展和现有多语言显示名；原 IPA 不得被改写。已安装的侧载 App 也应可修改名称，通过同一包标识符重新签名安装，使主屏幕名称和管理记录一致，而非仅改列表文字。无效标识、不可读取的元数据或用户取消必须停止安装并清理临时产物。修改包标识符后需按新身份保存独立安装记录并可刷新；不承诺第三方应用的登录、共享数据或推送通知兼容。
- `FR-048`：我的 App 安装进度须在列表顶部随内容滚动，不悬浮遮挡页面。安装因锁屏或切后台失去最终回执时，回前台优先用本机 UTI 正向证据恢复；未确认且有有效恢复记录时，通过已连接的新版 AltForge Server 查询设备上匹配的已安装包标识符，只在设备明确确认后补回管理记录并更新可见结果。Server 缺席、旧版或设备查询失败时保留记录和缓存，给出可操作提示，不把 App ID 或未确认回执当作安装成功；不承诺 iOS 无限后台执行。
- `FR-049`：macOS AltForge Server 菜单须常显独立的“安装 IPA”入口；iOS 导入 IPA 时同样可检查名称、包标识符、版本及扩展数量。两端均可直接安装，或修改名称、包标识符及自定义图标后安装。macOS 从 PNG/JPEG 文件选择并支持拖动、缩放方形裁切；iOS 从照片选择并使用系统方形裁切。进度显示所选应用的名称与可读取图标，无法读取时使用通用占位图。编辑只作用于临时解包副本，先于 provisioning profile；取消、失败与结束均清理临时资源，不改写原 IPA。第三方登录、共享数据和推送通知兼容性不作保证。
