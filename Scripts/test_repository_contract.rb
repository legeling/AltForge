...
assert(read(root, "Scripts/verify_apple_release_artifacts.sh").include?("check_release_privacy.py"), "Apple artifacts must match reviewed privacy declarations")
assert(workflow.include?("Scripts/test_release_privacy.py"), "release privacy regression fixtures must be a prepare gate")
privacy_gate = workflow.index("Verify generated source privacy against IPA")
assert(privacy_gate && privacy_gate < workflow.index("Publish GitHub Release assets"), "generated source privacy must be verified before Draft publication")
assert(workflow.include?("testUndeclaredPermissionsPresentationIsActionable"), "release CI must verify actionable error 201 copy")
...
server_storyboard = REXML::Document.new(read(root, "AltServer/Base.lproj/Main.storyboard"))
ipa_menu = REXML::XPath.first(server_storyboard, "//menuItem[@id='x0e-zI-0A2']")
assert(ipa_menu && ipa_menu.attributes['title'] == 'Install IPA…', "macOS must expose an independent Install IPA menu")
assert(!app_delegate.include?("sideloadAppMenuItem.isAlternate = true"), "Install IPA must not require the Option key")
assert(app_delegate.include?("reviewIPA(application)"), "local IPA installation must review metadata before authentication")
assert(app_delegate.include?("case .unchanged:") && app_delegate.include?("case .edited(let changes, let iconURL):"), "IPA review must offer direct and edited installation")
assert(app_delegate.include?("IPAIdentityEditor.apply(changes, to: application, within: temporaryDirectoryURL)"), "desktop edits must reuse the shared temporary-app editor")
assert(app_delegate.include?("IPAIconEditor.applyCustomIcon(at: iconURL"), "desktop edits must apply the selected icon before installation")
assert(app_delegate.include?("IPAIconEditor.preview(in: application.fileURL)"), "desktop IPA review must preview the selected app icon")
assert(app_delegate.include?("try? FileManager.default.removeItem(at: temporaryDirectoryURL)"), "local IPA preparation must clean its temporary directory")
ipa_icon_editor = read(root, "AltServer/IPAIconEditor.swift")
assert(ipa_icon_editor.include?('primary.removeValue(forKey: "CFBundleIconName")'), "a custom loose-file icon must replace the asset-catalog primary icon reference")
assert(ipa_icon_editor.include?('for key in ["CFBundleIcons", "CFBundleIcons~ipad"]'), "custom icons must cover both iPhone and iPad primary icons")
assert(ipa_icon_editor.include?('root.path.hasPrefix(temporaryRoot.path + "/")'), "custom icons must only mutate the temporary app copy")
assert(ipa_icon_editor.include?('static func cropSource(at url: URL)'), "custom icons must validate source images before cropping")
assert(ipa_icon_editor.include?('final class IPAIconCropView: NSView'), "desktop icon selection must include an interactive crop view")
assert(ipa_icon_editor.include?('"@3x.png"') && ipa_icon_editor.include?('"@2x~ipad.png"'), "custom icons must include phone and tablet variants")
my_apps = read(root, "AltStore/My Apps/MyAppsViewController.swift")
assert(my_apps.include?('picker.allowsEditing = true') && my_apps.include?('pendingSideloadIconSelection'), "iOS IPA import must offer photo cropping")
assert(my_apps.include?('IPAIconEditor.applyCustomIcon(at: iconURL'), "iOS IPA import must apply icons before installation")
assert(read(root, "AltStore/Info.plist").include?("NSPhotoLibraryUsageDescription"), "iOS photo icon selection must disclose library access")
assert(installation_manager.include?("if let preparedAppBundleURL"), "desktop installation must reuse the reviewed app instead of unzipping twice")
assert(installation_manager.index("if let preparedAppBundleURL") < installation_manager.index("prepareAllProvisioningProfiles(for: application"), "IPA edits must precede provisioning")
