...
| **IPA 信息编辑** | 在手机或 Mac 导入 IPA 后可核对信息、直接安装，或在签名前修改名称、包名和图标。手机可从照片选择并裁切，Mac 可选择 PNG/JPEG 图片并裁切。已管理应用可保持包名不变并重装改名；改包名可能影响登录、共享数据和推送。 |
...
| **可靠桌面安装** | macOS 菜单将“安装 IPA”与“安装 AltForge”分开显示。选择本地 IPA 后，先检查应用信息，再选择直接安装或在临时副本中修改名称、包标识符和可裁切图标；原 IPA 不变。安装过程继续显示下载、签名和设备进度。 |
